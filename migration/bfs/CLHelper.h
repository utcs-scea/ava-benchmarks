...
extern float init_time, mem_alloc_time, h2d_time, kernel_time,
      d2h_time, close_time, total_time, h2d_phy_time, d2h_phy_time;
...
void _clMemcpyH2D(cl_mem d_mem, int size, const void *h_mem_ptr)
{
    oclHandles.cl_status = clEnqueueWriteBuffer(oclHandles.queue, d_mem,
            CL_TRUE, 0, size, h_mem_ptr, 0, NULL, NULL);
#ifdef ERRMSG
    if(oclHandles.cl_status != CL_SUCCESS)
...
{
    oclHandles.cl_status = clEnqueueReadBuffer(oclHandles.queue, d_mem, CL_TRUE, 0, size, h_mem, 0,0, NULL);
#ifdef ERRMSG
