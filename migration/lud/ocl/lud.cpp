...
static cl_kernel diagnal;
static cl_kernel perimeter;
static cl_kernel internal;
...
{
	// release resources
	clReleaseKernel(diagnal);
	clReleaseKernel(perimeter);
	clReleaseKernel(internal);
...
main ( int argc, char *argv[] )
{
	...
	err = clEnqueueWriteBuffer(cmd_queue, d_m, 1, 0, matrix_dim*matrix_dim*sizeof(float), m, 0, 0, NULL);
	...
	for (i=0; i < matrix_dim-BLOCK_SIZE; i += BLOCK_SIZE) {
	  ...
	  err = clEnqueueNDRangeKernel(cmd_queue, diagnal, 2, NULL, global_work1, local_work1, 0, 0, NULL);
      ...
      if (global_work2[0] > 0) {
          err = clEnqueueNDRangeKernel(cmd_queue, perimeter, 2, NULL, global_work2, local_work2, 0, 0, NULL);
      ...
      // Intel OCL does not support 0 size
      if (global_work3[0] > 0) {
          err = clEnqueueNDRangeKernel(cmd_queue, internal, 2, NULL, global_work3, local_work3, 0, 0, NULL);
	...
	err = clEnqueueReadBuffer(cmd_queue, d_m, 1, 0, matrix_dim*matrix_dim*sizeof(float), m, 0, 0, NULL);
