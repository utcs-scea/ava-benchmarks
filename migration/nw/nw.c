...
static int initialize()
{
	...
	context = clCreateContextFromType( ctxprop, device_type, NULL, NULL, &ret);
	if( !context ) { printf("ERROR: clCreateContextFromType(%s) failed: %d\n", device_type == CL_DEVICE_TYPE_GPU ? "GPU" : "CPU"); return -1; }

	// create command queue for the specific device
#ifdef TIMING
	cmd_queue = clCreateCommandQueue( context, device_list[device_id_inuse], CL_QUEUE_PROFILING_ENABLE, &ret);
#else
	cmd_queue = clCreateCommandQueue( context, device_list[device_id_inuse], 0, &ret);
#endif
	if( !cmd_queue ) { printf("ERROR: clCreateCommandQueue() failed: %d\n", ret); return -1; }
...
int main(int argc, char **argv){
  ...
  printf("WG size of kernel = %d \n", BLOCK_SIZE);
	...
	size_t file_size = fread(source, 1, sourcesize, fp);
	...
	if(err != CL_SUCCESS) { printf("ERROR: clCreateKernel(kernel_nw1) 0 => %d\n", err); return -1; }
	kernel2 = clCreateKernel(prog, kernel_nw2, &err);  
	if(err != CL_SUCCESS) { printf("ERROR: clCreateKernel(kernel_nw2) 0 => %d\n", err); return -1; }
	...
	//write buffers
	err = clEnqueueWriteBuffer(cmd_queue, input_itemsets_d, 1, 0, max_cols * max_rows * sizeof(int), input_itemsets, 0, 0, NULL);
	if(err != CL_SUCCESS) { printf("ERROR: clEnqueueWriteBuffer bufIn1 (size:%d) => %d\n", max_cols * max_rows, err); return -1; }

	err = clEnqueueWriteBuffer(cmd_queue, reference_d, 1, 0, max_cols * max_rows * sizeof(int), reference, 0, 0, NULL);
    ...
    err = clEnqueueReadBuffer(cmd_queue, input_itemsets_d, 1, 0, max_cols * max_rows * sizeof(int), output_itemsets, 0, 0, NULL);
