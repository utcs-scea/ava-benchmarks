...
{
    fprintf(stderr,
        "Usage: %s <-n rows/cols> <-l layers> <-i iterations> <-f "
        "powerFile tempFile outputFile> [-p platform] [-d device]\n",
        argv[0]);
    fprintf(stderr,
        "\t<rows/cols>  - number of rows/cols in the grid (positive "
        "integer)\n");
    fprintf(stderr,
        "\t<layers>  - number of layers in the grid (positive "
        "integer)\n");

    fprintf(stderr, "\t<iteration> - number of iterations\n");
    fprintf(stderr,
        "\t<powerFile>  - name of the file containing the initial "
        "power values of each cell\n");
    fprintf(stderr,
        "\t<tempFile>  - name of the file containing the initial "
...
int main(int argc, char** argv)
{
    ...
    if (numCols == 0 || layers == 0 || iterations == 0
        || pfile == NULL) {
        usage(argc, argv);
    }
    ...
    float init_time = 0, mem_alloc_time = 0, h2d_time = 0,
          kernel_time = 0, d2h_time = 0, close_time = 0, total_time = 0;
    ...
    if (err != CL_SUCCESS || numPlatforms <= 0) {
        printf(
            "Error: Failed to get the platform!\n%s\n", err_code(err));
        return EXIT_FAILURE;
    }

    // get device
    cl_uint num_devices;
    err = clGetDeviceIDs(Platform[platform_id_inuse],
    ...
    d_a = clCreateBuffer(
        context, CL_MEM_READ_WRITE, sizeof(float) * count, NULL, NULL);
    d_b = clCreateBuffer(
    ...
    err = clEnqueueWriteBuffer(commands, d_a, CL_TRUE, 0,
        sizeof(float) * count, tIn, 0, NULL, &event);
    if (err != CL_SUCCESS) {
    ...
    err = clEnqueueWriteBuffer(commands, d_b, CL_TRUE, 0,
        sizeof(float) * count, pIn, 0, NULL, &event);
    if (err != CL_SUCCESS) {
    ...
    err = clEnqueueWriteBuffer(commands, d_c, CL_TRUE, 0,
        sizeof(float) * count, tempOut, 0, NULL, &event);
    if (err != CL_SUCCESS) {
    ...
    for (j = 0; j < iterations; j++) {
        ...
        for (int itr = 0; itr < 200; itr++)
            err = clEnqueueNDRangeKernel(
                commands, ko_vadd, 2, NULL, global, local, 0, NULL, NULL);
        if (err) {
    ...
    computeTempCPU(pIn, tempCopy, answer, numCols, numRows, layers, Cap,
        Rx, Ry, Rz, dt, amb_temp, iterations);
