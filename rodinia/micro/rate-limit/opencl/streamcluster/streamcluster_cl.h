...
Science Department of Software Technology Parallel and Distributed
Systems Group on 15/03/2010
***********************************************/
...
float pgain(long x, Points* points, float z, long int* numcenters,
    int kmax, bool* is_center, int* center_table,
    char* switch_membership, double* serial, double* cpu_gpu_memcpy,
    double* memcpy_back, double* gpu_malloc, double* kernel)
{
    float gl_cost = 0;
    try {
#ifdef PROFILE_TMP
        ...
        *serial += t2 - t1;
        ...
        /***** initial memory allocation and preparation for transfer :
         * execute once -1 *****/
        if (c == 0) {
#ifdef PROFILE_TMP
            double t3 = gettime();
#endif
            coord_h = (float*)malloc(
                num * dim * sizeof(float)); // coordinates (host)
            gl_lower = (float*)malloc(kmax * sizeof(float));
#ifdef TIMING
            gettimeofday(&tv_mem_alloc_start, NULL);
#endif
            work_mem_h
                = (float*)_clMallocHost(kmax * num * sizeof(float));
#ifdef TIMING
            ...
            p_h = (Point_Struct*)malloc(num
                * sizeof(Point_Struct)); // by cambine: not compatibal
                                         // with original Point

            // prepare mapping for point coordinates
            //--cambine: what's the use of point coordinates? for
            //computing distance.
            for (int i = 0; i < dim; i++) {
                for (int j = 0; j < num; j++)
        ...
        *cpu_gpu_memcpy += t8 - t7;
        ...
        /***** kernel execution *****/
        /* Determine the number of thread blocks in the x- and
         * y-dimension */
        ...
        *kernel += t10 - t9;
        ...
        _clMemcpyD2H(
            switch_membership, switch_membership_d, num * sizeof(char));
        _clMemcpyD2H(
            work_mem_h, work_mem_d, (K + 1) * num * sizeof(float));
        ...
        *memcpy_back += t11 - t10;
        ...
        /* if opening a center at x saves cost (i.e. cost is negative)
