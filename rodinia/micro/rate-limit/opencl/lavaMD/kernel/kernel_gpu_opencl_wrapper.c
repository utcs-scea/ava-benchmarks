							...
							FOUR_VECTOR* fv_cpu)
{
    ...
    h2d_phy_time += probe_event_time(event1, command_queue);
    clReleaseEvent(event1);
    h2d_phy_time += probe_event_time(event2, command_queue);
    clReleaseEvent(event2);
    h2d_phy_time += probe_event_time(event3, command_queue);
    clReleaseEvent(event3);
    h2d_phy_time += probe_event_time(event4, command_queue);
	...
	if (error != CL_SUCCESS) 
		fatal_CL(error, __LINE__);
    d2h_phy_time += probe_event_time(event1, command_queue);
    clReleaseEvent(event1);
