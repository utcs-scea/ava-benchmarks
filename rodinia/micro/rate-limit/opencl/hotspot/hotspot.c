...
int compute_tran_temp(cl_mem MatrixPower, cl_mem MatrixTemp[2], int col,
    ...
    int blockRows, int borderCols, int borderRows, float* TempCPU,
    float* PowerCPU)
{
    ...
    float Cap = FACTOR_CHIP * SPEC_HEAT_SI * t_chip * grid_width
        * grid_height;
    ...
    for (t = 0, itr = 0; t < total_iterations;
         t += num_iterations, itr++) {
        ...
        clSetKernelArg(
            kernel, 2, sizeof(cl_mem), (void*)&MatrixTemp[src]);
        clSetKernelArg(
            kernel, 3, sizeof(cl_mem), (void*)&MatrixTemp[dst]);
        ...
        error = clEnqueueNDRangeKernel(command_queue, kernel, 2, NULL,
            global_work_size, local_work_size, 0, NULL, &events[itr]);
        if (error != CL_SUCCESS)
...
void usage(int argc, char** argv)
{
    fprintf(stderr,
        ...
        "<temp_file> <power_file> <output_file> [-p platform_id] [-d "
        "device_id]\n",
        argv[0]);
    fprintf(stderr,
...
int main(int argc, char** argv)
{
    ...
    // Print the chosen platform (if there are multiple platforms,
    // choose the first one)
    cl_platform_id platform = platforms[platform_id_inuse];
    char pbuf[100];
    error = clGetPlatformInfo(
        platform, CL_PLATFORM_VENDOR, sizeof(pbuf), pbuf, NULL);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);
    printf("Platform: %s\n", pbuf);
    ...
    error = clGetDeviceIDs(
        platform, CL_DEVICE_TYPE_ALL, 0, NULL, &devices_size);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);
    ...
    cl_device_id* devices
        = (cl_device_id*)malloc(sizeof(cl_device_id) * devices_size);
    error = clGetDeviceIDs(
        platform, CL_DEVICE_TYPE_ALL, devices_size, devices, NULL);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);

    device = devices[device_id_inuse];
    error = clGetDeviceInfo(
        device, CL_DEVICE_NAME, sizeof(pbuf), pbuf, NULL);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);
    printf("Device: %s\n", pbuf);
    ...
    context = clCreateContextFromType(
        context_properties, device_type, NULL, NULL, &error);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);
    ...
    cl_program program = clCreateProgramWithSource(
        context, 1, &source, &sourceSize, &error);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);
...
#ifdef BLOCK_SIZE
    sprintf(
        clOptions + strlen(clOptions), " -DBLOCK_SIZE=%d", BLOCK_SIZE);
#endif
    ...
    // Create two temperature matrices and copy the temperature input
    // data
    cl_mem MatrixTemp[2];
    ...
    // CL_MEM_ALLOC_HOST_PTR, sizeof(float) * size, NULL, &error);
    MatrixTemp[1] = clCreateBuffer(
        context, CL_MEM_READ_WRITE, sizeof(float) * size, NULL, &error);
    ...
    MatrixTemp[0] = clCreateBuffer(context,
        CL_MEM_READ_WRITE | CL_MEM_USE_HOST_PTR, sizeof(float) * size,
        FilesavingTemp, &error);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);
    // Copy the power input data
    cl_mem MatrixPower = clCreateBuffer(context,
        CL_MEM_READ_ONLY | CL_MEM_USE_HOST_PTR, sizeof(float) * size,
        FilesavingPower, &error);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);

// https://software.intel.com/en-us/forums/opencl/topic/509406
// The copy should happen when the data is used; however the test shows
// the data is copied immediately with
//  nVidia OpenCL. w/o we include the time as h2d_time.
    ...
    int ret = compute_tran_temp(MatrixPower, MatrixTemp, grid_cols,
        grid_rows, total_iterations, pyramid_height, blockCols,
        blockRows, borderCols, borderRows, FilesavingTemp,
        FilesavingPower);

    cl_event event;
    // Copy final temperature data back
    cl_float* MatrixOut = (cl_float*)clEnqueueMapBuffer(command_queue,
        MatrixTemp[ret], CL_TRUE, CL_MAP_READ, 0, sizeof(float) * size,
        0, NULL, &event, &error);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);
#ifdef TIMING
