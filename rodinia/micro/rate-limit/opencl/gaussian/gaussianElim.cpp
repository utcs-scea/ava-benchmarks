...
int main(int argc, char *argv[]) {
    ...
    // parse command line
    if (parseCommandline(argc, argv, filename, &quiet, &show_data,
            &timing, &platform, &device, &size)) {
        printUsage();
        return 0;
    }
 ...
 **------------------------------------------------------
 */
void ForwardSub(cl_context context, float* a, float* b, float* m,
    int size, int timing)
{
    // 1. set up kernels
    ...
    error = clEnqueueWriteBuffer(command_queue,
               ...
               NULL,
               &writeEvent);
    h2d_phy_time += eventTime(writeEvent,command_queue);
    ...
    d2h_phy_time +=eventTime(readEvent,command_queue);
