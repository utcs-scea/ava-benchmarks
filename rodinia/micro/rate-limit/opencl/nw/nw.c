...
static int initialize()
{
    ...
    printf("create %lx context err=%d\n", (uintptr_t)platform_id, ret);
    if ( !context )
    {
...
int main(int argc, char **argv)
{
    ...
    h2d_phy_time += probe_event_time(event[0], cmd_queue);
    ...
    h2d_phy_time += probe_event_time(event[1], cmd_queue);
    ...
    int blk_tmp = 1;
    global_work[0] = BLOCK_SIZE * blk_tmp;
    local_work[0]  = BLOCK_SIZE;
    clSetKernelArg(kernel1, 7, sizeof(cl_int), (void*) &blk_tmp);
    ...
    d2h_phy_time += probe_event_time(event[0], cmd_queue);
    clReleaseEvent(event[0]);
